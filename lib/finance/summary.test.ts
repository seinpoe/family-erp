...
describe("finance summary", () => {
  it("summarizes same-currency financial records without treating transfers as spend", () => {
    ...
    ])).toEqual({ currency: "USD", hasMultipleCurrencies: false, netPosition: 1999.5, income: 2500, expenses: 425.5, liabilities: 75, recordCount: 4 });
  });
  ...
  it("does not fabricate a combined balance across different currencies", () => {
    ...
    ])).toEqual({ currency: null, hasMultipleCurrencies: true, netPosition: null, income: null, expenses: null, liabilities: null, recordCount: 2 });
  });
});
