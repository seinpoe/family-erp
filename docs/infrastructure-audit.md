...
The local 390 × 844 verification showed that the prior mobile shell was crowded by theme, security, and sign-out controls, while its full-screen placeholder did not provide a clear recovery path. The shell now presents a compact brand header and a fixed, safe-area-aware five-item navigation bar. The client-side theme preference has moved from the permanently mounted shell to the Account workspace, so it is not rendered on everyday dashboard and module routes. The loading route retains the accessible busy landmark but presents only a concise household summary placeholder; after 4.5 seconds it explains that loading has taken longer than usual and exposes an explicit refresh control.
